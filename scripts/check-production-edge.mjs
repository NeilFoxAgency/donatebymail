...
export function validateProductionEdgeSettings(settings = []) {
  ...
  if (values.get("waf") !== "on") errors.push("Cloudflare managed WAF must be enabled.");
...
export { readZoneSettings };

if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  try {
    const settings = await readZoneSettings(process.env.CLOUDFLARE_API_TOKEN);
    const errors = validateProductionEdgeSettings(settings);
    if (errors.length) {
