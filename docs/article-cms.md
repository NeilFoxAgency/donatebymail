...
- Immediate publication remains a separate semantic command. It must name an
  existing revision belonging to the target article and cannot be smuggled
  into a draft or schedule request.
...
The beta policy can be versioned again if publication review is reintroduced.
No production Worker or production database resource is changed by this
branch.
