...
| Functional mission actions | The estimator, multi-device donor packet, packing slip, acknowledgment preview, draft saving, and contact handoff are functional. |
...
- Prepared event names include `donation_packet_created`, `prepaid_label_request_started`, `contact_email_started`, and `nonprofit_partner_inquiry_started`.
...
4. Confirm real prepaid-label or mailed-kit requests can be fulfilled consistently. A carrier-label integration is preferable before sending large paid-traffic volumes.
