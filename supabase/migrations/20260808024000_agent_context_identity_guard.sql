-- Optional donor/partner context must be identity-verified before it can
-- make a low-risk automatic message eligible. Without this guard, a caller
-- could attach an arbitrary context UUID to a generic FAQ/setup message and
-- bypass the trusted-inbound requirement.
...
declare
  ...
  donation_identity_ok boolean := candidate_donation_id is null;
  organization_identity_ok boolean := candidate_organization_id is null;
  campaign_context_ok boolean := candidate_campaign_id is null;
  context_supplied boolean := candidate_donation_id is not null
    or candidate_organization_id is not null
    or candidate_campaign_id is not null;
...
begin
  ...
  if candidate_organization_id is not null then
    organization_identity_ok := exists(
      select 1
      ...
      select 1 from app_private.organizations o
      join app_private.partner_invitations i on i.organization_id = o.id
      where o.id = candidate_organization_id and o.status = 'active'
  ...
  if candidate_campaign_id is not null then
    campaign_context_ok := candidate_organization_id is not null and exists(
      select 1 from app_private.campaigns cp
      where cp.id = candidate_campaign_id
        and cp.organization_id = candidate_organization_id
    );
  end if;
  identity_ok := donation_identity_ok and organization_identity_ok and campaign_context_ok
    and (context_supplied or not identity_required);
  ...
  if normalized_category = 'internal_escalation'
    ...
    risk_value := 'moderate';
  elsif not safe_category or (identity_required and not identity_ok)
    or (context_supplied and not identity_ok) then
    risk_value := 'moderate';
  else
    risk_value := 'low';
  end if;
  ...
  if existing_decision.id is not null then
    ...
    return jsonb_build_object(
      ...
      'autoSendAllowed', existing_decision.outcome = 'ALLOW_AUTOMATICALLY'
        and existing_authorization.consumed_at is null
        and existing_authorization.expires_at > now(),
      'replayed', true
    );
  end if;

  if risk_value = 'low' and safe_category and (not identity_required or identity_ok)
    and (not context_supplied or identity_ok) then
    ...
    rationale_value := case
      ...
      when context_supplied and not identity_ok then 'support_context_not_verified'
...
begin
  if new.outcome = 'ALLOW_AUTOMATICALLY'
    and (
      ((new.donation_id is not null or new.organization_id is not null or new.campaign_id is not null)
        and new.identity_verified is not true)
      or (new.donation_id is null and new.organization_id is null and new.campaign_id is null
