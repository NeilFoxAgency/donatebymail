...
export function DonorAccountPage() {
  ...
  useEffect(() => { sessionStatus("/api/account/session").then(setAuthenticated).catch(() => setAuthenticated(false)); }, []);
  useEffect(() => { if (authenticated) load().catch((error) => { setMessage(error.message); setAuthenticated(false); }); }, [authenticated]);
  if (!authenticated) return <main className="operations-main"><section className="operations-shell compact"><p className="kicker">Donor account</p><h1>Track your claimed donations</h1><p>We’ll send a one-time secure sign-in link. A donation is linked only after you explicitly claim its one-time reference.</p><form onSubmit={async (event) => { event.preventDefault(); const response = await fetch("/api/account/auth/magic-link", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email }) }); const body = await response.json() as { message: string }; setMessage(body.message); }}><label>Email<input type="email" autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} required /></label><button className="button primary">Email secure sign-in link</button></form>{message && <p role="status">{message}</p>}</section></main>;
...
function ClaimForm({ onClaimed }: { onClaimed: () => Promise<void> }) {
  ...
  return <form className="inline-ops" onSubmit={async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); await authenticatedApi("/api/account/claims", { method: "POST", body: JSON.stringify({ publicId: form.get("publicId"), claimToken: form.get("claimToken") }) }); await onClaimed(); }}><h2>Claim a donation</h2><label>Donation ID<input name="publicId" defaultValue={params.get("donation") || ""} required /></label><label>One-time claim reference<input name="claimToken" defaultValue={params.get("claim") || ""} autoComplete="off" required /></label><button>Claim donation</button></form>;
}
